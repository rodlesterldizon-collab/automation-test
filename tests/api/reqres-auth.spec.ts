...
import { RegisterSuccessResponse, LoginSuccessResponse, ErrorResponse } from './helpers/reqres.types';

test.describe('ReqRes Authentication API Tests', () => {

  test.describe('Registration Endpoints', () => {
    test('[REQ-A01] Should successfully register a valid user - @smoke', async ({ request }) => {
    ...
    test('[REQ-A02] Should fail registration when password is missing', async ({ request }) => {
    ...
    test('[REQ-A03] Should fail registration when email is missing', async ({ request }) => {
    ...
    test('[REQ-A04] Should fail registration with an empty payload', async ({ request }) => {
    ...
    test('[REQ-A05] Should fail registration with an unregistered email', async ({ request }) => {
  ...
  test.describe('Login Endpoints', () => {
    test('[REQ-A06] Should successfully login a valid user - @smoke', async ({ request }) => {
    ...
    test('[REQ-A07] Should fail login when password is missing', async ({ request }) => {
    ...
    test('[REQ-A08] Should fail login when email is missing', async ({ request }) => {
    ...
    test('[REQ-A09] Should fail login with an empty payload', async ({ request }) => {
      // Given an empty payload
      const payload = {};
    ...
    test('[REQ-A10] Should fail login with an unregistered email', async ({ request }) => {
      // Given an email not found in the ReqRes system
      const payload = { email: 'unknown@reqres.in', password: 'password123' };
